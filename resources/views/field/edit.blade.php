...
<div class="main-content-inner">
   ...
   <div class="row">
       <div class="col-sm-12">
           <div class="card mt-5">
               <div class="card-body">
                   <form action="{{route('field.update', $data->id)}}" method="post" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            <label for="name">Nama Lapangan</label>
                            <input type="text" class="form-control @error('name') invalid @enderror" id="name" name="name" placeholder="Masukkan Nama Lapangan" required value="{{$data->name}}">
                            @error('name') <small class="form-text text-danger">{{$errors->first('name')}}</small> @enderror
                        </div>

                        <div class="form-group">
                            <label for="price">Price / hour</label>
                            <input type="text" class="form-control @error('price') invalid @enderror" id="price" name="price" placeholder="Price / hour" required value="{{$data->price}}" onkeyup="rupiah(this.value)">
                        ...
                        <img src="{{asset($data->photo)}}" alt="" width="100" height="70" style="border-radius: 20px"> <br>
